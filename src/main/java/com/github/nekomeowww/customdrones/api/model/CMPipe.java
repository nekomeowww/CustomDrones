        ...
        extends CMBase
{
    ...
    {
        this(e1, e2, e1.subtract(e2), e1.subtract(e2), radius, radius, sides);
    }

    public CMPipe(Vec3d e1, Vec3d e2, double radius1, double radius2, int sides)
    {
        this(e1, e2, e1.subtract(e2), e1.subtract(e2), radius1, radius2, sides);
    }
    ...
    public CMPipe(Vec3d e1, Vec3d e2, Vec3d p1, Vec3d p2, double end1r, double end2r, int s)
    {
        ...
        this.axis1 = p1;
        this.axis2 = p2;
        if (this.axis1.dotProduct(end12) > 0.0D) {
            this.axis1 = scale(this.axis1, -1.0D);
        }
        if (this.axis2.dotProduct(end12) > 0.0D) {
    ...
    public CMPipe autoUVfromU(double u1, double v1, double u2)
    {
        ...
        double totalHeight = Math.max(this.ra1, this.ra2) * 2.0D + end12.lengthVector();
    ...
    {
        Vec3d end12 = fromTo(this.end1, this.end2);
        Vec3d mid12 = scale(this.end1.add(this.end2), 0.5D);
        double angleRad = 6.283185307179586D / this.sides;
        double sideW1 = Math.tan(angleRad / 2.0D) * this.ra1 * 2.0D;
        double sideW2 = Math.tan(angleRad / 2.0D) * this.ra2 * 2.0D;
        double totalWidth = (sideW1 + sideW2) / 2.0D * this.sides;
        ...
        {
            v2 = v1;
            totalHeight = end12.lengthVector();
        }
        ...
        if (spin1.dotProduct(spin2) < 0.0D) {
        ...
        for (int a = 0; a < this.sides + 1; a++)
        {
            ...
            Vec3d final2 = setLength(rotateAround(spin2, this.axis2, this.initSpin2 + 6.283185307179586D / this.sides * a), this.ra2).add(this.end2);
            Vec3d final2ToLater = fromTo(final2,
                    setLength(rotateAround(spin2, this.axis2, this.initSpin2 + 6.283185307179586D / this.sides * (a + 1)), this.ra2).add(this.end2));
            Vec3d final1 = setLength(rotateAround(spin1, this.axis1, this.initSpin1 + 6.283185307179586D / this.sides * a), this.ra1).add(this.end1);
            Vec3d normal = vec(0.0D, 1.0D, 0.0D);
            if (final2ToLater.lengthVector() == 0.0D)
            {
                Vec3d final1ToLater = fromTo(final1,
                        setLength(rotateAround(spin1, this.axis1, this.initSpin1 + 6.283185307179586D / this.sides * (a + 1)), this.ra1)
                                .add(this.end1));
                normal = getPerpendicularVec(final1ToLater, fromTo(final1, final2)).normalize();
            }
            else
            {
                normal = getPerpendicularVec(fromTo(final2, final1), final2ToLater).normalize();
            }
            normal(normal.xCoord, normal.yCoord, normal.zCoord);
            vertex(final2.xCoord, final2.yCoord, final2.zCoord, unow, v2);
            vertex(final1.xCoord, final1.yCoord, final1.zCoord, unow, v4);
        }
        end();
        if (this.renderCaps != CapType.NONE)
        {
            ...
            {
                begin(9);
                normal(this.axis1.xCoord, this.axis1.yCoord, this.axis1.zCoord);
                for (int a = 0; a < this.sides; a++)
                {
                    Vec3d botTex = rotateAround(texSpin, zUnit, 6.283185307179586D / this.sides * a);

                    botTex = vec(botTex.xCoord * (totalWidth / totalHeight * (v4 - v1) / (uend - u1)), botTex.yCoord, botTex.zCoord).add(texMid1);

                    Vec3d botVertex = setLength(rotateAround(spin1, this.axis1, this.initSpin1 - 6.283185307179586D / this.sides * a), this.ra1).add(this.end1);
                    vertex(botVertex.xCoord, botVertex.yCoord, botVertex.zCoord, botTex.xCoord, botTex.yCoord);
                }
                end();
            }
            if (this.renderCaps != CapType.BOTTOM)
            {
                ...
                begin(9);
                normal(this.axis2.xCoord, this.axis2.yCoord, this.axis2.zCoord);
                for (int a = 0; a < this.sides; a++)
                {
                    Vec3d topTex = rotateAround(texSpin, zUnit, 6.283185307179586D / this.sides * a);

                    topTex = vec(topTex.xCoord * (totalWidth / totalHeight * (v4 - v1) / (uend - u1)), topTex.yCoord, topTex.zCoord).add(texMid2);

                    Vec3d topVertex = setLength(rotateAround(spin2, this.axis2, this.initSpin2 + 6.283185307179586D / this.sides * a), this.ra2).add(this.end2);
                    vertex(topVertex.xCoord, topVertex.yCoord, topVertex.zCoord, topTex.xCoord, topTex.yCoord);
                }
                end();
            }
        }
    ...
    public void newRender()
    {
        if ((this.end1Vers.isEmpty()) || (this.end2Vers.isEmpty()) || (this.sideVers.isEmpty()))
        {
            ...
            Vec3d zUnit = vec(0.0D, 0.0D, 1.0D);
            Vec3d texSpin = vec(0.0D, (v2 - v1) / 2.0D, 0.0D);
            boolean flipNormal1 = this.axis1.dotProduct(end12) > 0.0D;
            boolean flipNormal2 = this.axis2.dotProduct(end12) < 0.0D;
            for (int a = 0; a < this.sides; a++)
            {
                Vec3d botTex = rotateAround(texSpin, zUnit, 6.283185307179586D / this.sides * a);
